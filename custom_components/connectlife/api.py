    ...
    async def _initial_access_token_with_retry(self) -> None:
        ...
        for profile_index, profile in enumerate(profiles, start=1):
            ...
            for attempt in range(1, attempts + 1):
                try:
                    await self._initial_access_token(profile)
                    self._active_oauth_profile = profile
                    return
                except (aiohttp.ClientError, TimeoutError) as err:
                    last_error = LifeConnectAuthError(
        ...
        if last_error is not None:
            raise cast(LifeConnectAuthError, last_error)
        raise LifeConnectAuthError("Unexpected error during login")

    async def _initial_access_token(self, profile: OAuthClientProfile) -> None:
        async with self._client_session() as session:
            uid, login_token = await self._login_to_gigya(session)
            id_token = await self._fetch_jwt(session, login_token)
            code = await self._authorize(session, uid, id_token, profile)
            await self._exchange_authorization_code(session, code, profile)
